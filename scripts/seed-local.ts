...
async function seedLocal() {
	try {
		...
		// SQLファイルを生成
		const sqlFile = join(process.cwd(), 'temp-seed.sql');
		
		// バッチサイズで処理
		const batchSize = 50; // SQLコマンドの制限を考慮して小さめに
		let totalInserted = 0;
		let sqlContent = '';
		...
		// wrangler d1 executeを使って実行
		console.log('データベースに投入中...');
		try {
			execSync(`wrangler d1 execute atcoder-db --local --file="${sqlFile}"`, {
				stdio: 'inherit',
				shell: true,
				cwd: process.cwd()
			});
			totalInserted = data.length;
			console.log(`✅ ${totalInserted}件の問題を投入しました！`);
		} catch (error) {
